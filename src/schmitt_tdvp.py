...
from netket.utils.types import Sequence
...
from tdvp_utils import make_monitor_dict
...
def _impl(parameters, n_samples, E_loc, S, rhs_coeff, rcond, rcond_smooth, snr_atol):
    ...
    update = V @ eta_p
    update = jnp.nan_to_num(update, nan=0.)
    ...
    ev_reg = jnp.where(ev_inv * regularizer2 < 1. / rcond, 1./ (ev_inv * regularizer2), jnp.nan)

    return E, dw, rmd, snr, snr_F, ev, ev_reg


@odefun.dispatch
def odefun_custom(state: MCState, self: TDVPSchmitt, t, w, *, stage=0):  # noqa: F811
    ...
    self._monitor = make_monitor_dict(
        self._rmd, np.nan, self._snr, self._snr_F, self._ev, self._ev_reg
    )
