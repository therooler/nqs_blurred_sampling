...
from tdvp_utils import make_monitor_dict, randomized_permutation_bridge_sample, ess_from_weights
...
class TDVPSchmittRandomizedPermutationBridge(TDVPBaseDriver):
    r"""
    ...
    def __init__(
        ...
        *,
        q_hamiltonian: float = 0.2,
        q_permutation: float = 0.2,
        t0: float = 0.0,
        ...
        if not (0 < q_hamiltonian + q_permutation < 1):
            raise ValueError(f"`q_hamiltonian + q_permutation` must satisfy 0 < q_hamiltonian + q_permutation < 1, received {q_hamiltonian + q_permutation}")
        self.q_permutation = q_permutation
        self.q_hamiltonian = q_hamiltonian
        self.q = [q_permutation, q_hamiltonian]
...
@odefun.dispatch
def odefun_custom(
    state: MCState, self: TDVPSchmittRandomizedPermutationBridge, t, w, *, stage=0
):  # noqa: F811
    ...
    samples_q, importance_weights, E_loc = HashablePartial(
        randomized_permutation_bridge_sample,
        apply_fn=state._apply_fun,
        op=op_t,
        q_hamiltonian=self.q_hamiltonian,
        q_permutation=self.q_permutation,
