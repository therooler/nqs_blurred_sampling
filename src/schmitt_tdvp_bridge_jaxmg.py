...
class TDVPSchmittBridgeJAXMg(TDVPBaseDriver):
    r"""
    ...
    def __init__(
        ...
        *,
        ...
        mode = nkjax.jacobian_default_mode(
            variational_state._apply_fun,
            variational_state.parameters,
            variational_state.model_state,
            variational_state.samples,
            holomorphic=False,
        )
        assert (
            mode == "complex"
        ), f"This driver is only implemented for complex wave functions functions, received mode={mode}"
        super().__init__(
...
@partial(
    jax.jit,
    static_argnames=("n_samples", "rhs_coeff", "rcond", "rcond_smooth", "snr_atol"),
)
def _impl(
    parameters,
    n_samples,
    ...
    O_LT = O_L.reshape(-1, O_L.shape[-1])
    O_LT = jax.lax.with_sharding_constraint(
        O_LT,
        NamedSharding(mesh, P("S", None)),
    )
    ΔE_loc = jax.lax.with_sharding_constraint(
        ΔE_loc,
    ...
    # Calculate S-matrix
    S_L = O_LT.T @ O_LT
    S_L = jax.lax.with_sharding_constraint(S_L, P("S", None))

    # Prepare Jacobian
    O_L = O_L.reshape(-1, 2, O_L.shape[-1])
    O_L = O_L[:, 0, :] + 1j * O_L[:, 1, :]
    O_L = O_L.reshape(-1, O_L.shape[-1])
    O_L = O_L * jnp.sqrt(importance_weights / importance_weights.shape[0])

    ev, V = jnp.linalg.eigh(S_L)

    if JAXMG_ENABLED:
        ev, V = syevd(S_L, T_A=1024, mesh=mesh, in_specs=(P("S", None),))
    else:
        ev, V = jnp.linalg.eigh(S_L)
    OEdata = O_L.conj() * ΔE_loc
    # SNR of the force estimator F = sum_i O_i^* ΔE_i
    OE_mean = stats.mean(OEdata, axis=0)
    OE_var = stats.var(OEdata, axis=0)
    eps = jnp.finfo(O_L.dtype).eps
    snr_F = jnp.where(
        OE_var <= eps,
        jnp.inf,
    ...
    Q = jnp.tensordot(V.conj().T, O_L.T, axes=1).T
    ...
    rmd = jnp.linalg.norm(S_L @ update - rhs_coeff * F) / jnp.linalg.norm(F)

    y, reassemble = convert_tree_to_dense_format(parameters, "complex")
...
@odefun.dispatch
def odefun_custom(
    state: MCState, self: TDVPSchmittBridgeJAXMg, t, w, *, stage=0
):  # noqa: F811
    ...
    #     chunk_size=chunk_size,
    # )
    # _, unravel_params_fn = ravel_pytree(state.parameters)
    ...
    # print(self._S.O[0] / O_L[0])
    # assert jnp.allclose(self._S.O[0:10], O_L[0:10])
    # from schmitt_tdvp_bridge import _impl as _impl_prev
...
def prepare_input(
    log_psi, parameters, samples, pdf, model_state, mode: str, chunk_size: int
):
    if samples.ndim >= 3:
